...
RA2A 创建 turn 时优先交给当前 Codex Desktop owner，包括目标空闲的情况，从而保持已打开的 Desktop UI 实时同步。只有 Desktop 明确确认请求未投递时，才回退到受管 Codex App Server。owner-first 路径已完成 Windows 真实验收和 macOS 20 轮跨设备 soak，未出现 UI 刷新退化。
