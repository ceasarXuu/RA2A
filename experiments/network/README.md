...
|---|---:|---:|
| 相同预共享凭证建立连接并回显消息 | 通过 | 通过 |
| 不同预共享凭证拒绝连接 | 通过 | 通过 |
| 6 位 PIN 原样作为 PSK | 不适用，`pnet` 要求 32 字节 | 通过 |
...
第一版直接把长期 6 位 PIN 的原始字节交给 Pion DTLS 作为 PSK，不做 KDF、证书、PAKE 或额外加密封装。这已通过相同 PIN 消息往返、不同 PIN 拒绝连接和 20 KB CoAP 分块传输测试。

这是用于先跑通闭环的简单凭证，不是高强度安全设计。6 位 PIN 容易被猜测，配置文件中的 PIN 也不使用系统密钥链；第一版不承诺抵抗恶意局域网参与者、凭证窃取或穷举攻击。
...
本阶段依据 PD17 将凭证方案收敛为“6 位 PIN 原样作为 DTLS-PSK”，属于用户直接确认的产品决策变更。节点仍然只有“列出 session”和“投递消息”两个接口。CoAP 对同一 Confirmable Message ID 的传输重发由接收库去重，不得被实现成新的应用层 `POST`，从而继续遵守 PD13 的“结果未知不自动再次投递”。
