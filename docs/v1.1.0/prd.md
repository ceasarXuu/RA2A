...
尚待确认：Codex CLI 的托管方式、目标地址演进方式，以及“正式支持一种 Agent”的验收口径。因此本文暂为 Draft。
...
1. 用户在设备上启动 RA2A 服务。
2. RA2A 加载本机可用的 Agent 适配器，并汇总可投递端点。
3. 远端 Agent 调用 `list_targets`，获得完整、可直接使用的目标地址和必要能力信息。
4. 远端 Agent 调用 `send_message`，RA2A 将统一消息路由到目标端点所属适配器。
5. 目标适配器将消息转换为宿主能接受的操作，并返回统一投递结果。
6. 失败时，调用方获得稳定且与宿主无关的错误语义；诊断信息可以保留宿主细节。
...
- 无法可靠识别来源 session 时，不得静默伪造身份；应降级为明确的节点/Agent 来源，或拒绝发送，具体行为待确认。
...
推荐方案：RA2A 启动受管 App Server，用户通过 `codex --remote` 连接；RA2A 和 TUI 共享同一个会话所有者。这最有机会避免 writer 抢占和 TUI 状态失真，但会改变 Codex CLI 的启动方式。

备选方案：直接向任意已运行的 Codex CLI session 注入。用户体验更自然，但需要实验证明官方入口能在活跃 TUI 下安全投递并同步状态。

需要确认 v1.1.0 是只承诺受管 CLI session，还是必须支持任意已运行 session。

### Q2：目标地址是否显式包含 Agent 类型

推荐保持地址对调用方不透明：`list_targets` 返回完整地址，内部端点 ID 编码适配器类型，调用方不得自行拼接。

备选方案是升级为 `ra2a://<node>/<agent-type>/<session-id>`。它更直观，但会改变当前地址格式，并要求明确兼容策略。

### Q3：“正式支持一种 Agent”的最低口径

建议定义为：可发现、可作为发送方、可作为接收方、可返回统一投递结果、可在投递后继续人工交互、通过与所有已支持 Agent 的交叉测试。

需要 Owner 确认该口径是否作为后续 Agent 支持列表的统一门槛。
